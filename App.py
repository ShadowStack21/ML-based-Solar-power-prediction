...
def generate_solar_data():
    ...
    def seasonal_irradiation(month):
        if month in [3,4,5,6]:     return np.random.uniform(750, 1000) 
        elif month in [7,8,9]:     return np.random.uniform(300, 600)  
        elif month in [10,11]:     return np.random.uniform(500, 800)  
        else:                      return np.random.uniform(400, 700)  
